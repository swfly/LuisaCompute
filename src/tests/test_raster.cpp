...
int main(int argc, char *argv[]) {
    ...
    auto shader = raster_ext->compile(
        mesh_format,
        kernel);
    PixelFormat formats[] = {out_img.format()};
    auto scene = raster_ext->create_scene(formats, depth_buffer.format());
    ...
    BufferView<Vertex> vbv[] = {vert_buffer};
    RasterState state{
        ...
        .conservative = true};
    scene.emplace_back(
        luisa::span<BufferView<Vertex> const>{vbv},
        BufferView<uint>(idx_buffer), 1, state, shader,
        /*vertex arguments*/ 5.0f,
        /*index arguments*/ 3.0f);
    float4x4 view;
    float4x4 proj;
    ImageView<float> imgs[] = {out_img};
    stream << scene.build()
           << scene.draw(
                  view,
                  proj,
